...
. *Offsets or not:* Is the included data only line and column? Or should the offset within the file be included as well?
. *Zero-based or one-based:* Should the offset/line/column be zero-based or one-based? The answer isn't inherently the same for all 3 elements.
. *Inclusive or exclusive:* Are the start and end inclusive or exclusive of the matched characters? Again this may differ between start and end.
. *Special representation:* Does a newline character (or any other character)  have special representation in the format?
...
The location tracking in use in pre-spec is defined in implementation of the asciidoc-tck and asciidoc-parsing-lab and discussed in the following to issue threads:
...
. **Purpose/Goals:** So far I haven't found a direct articulation of this but the following are the ones that seemed most informative to me:
+
[quote]
____
...
. **Offsets or not:** Currently, I've seen no proposal that includes an offset within the `location` property.
...
. **Special handling:** Locations that represent the newline character of an empty line have a column of 0, while in all other cases the first character of a line is considered to have column 1.
